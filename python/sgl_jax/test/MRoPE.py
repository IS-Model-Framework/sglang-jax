from sgl_jax.srt.layers.embeddings import RotaryEmbedding, _apply_rotary_emb
import math
...
class MRotaryEmbedding(RotaryEmbedding):
    ...
    def _forward_mrope(
        ...
        if self.mrope_interleaved:
            ...
            indices = [0] + list(self.split_indices) + [cos_all.shape[-1]]
            cos = jnp.concatenate([
                cos_all[i, :, indices[i]:indices[i+1]] 
                for i in range(len(self.mrope_section))
            ], axis=-1)

            sin = jnp.concatenate([
                sin_all[i, :, indices[i]:indices[i+1]] 
                for i in range(len(self.mrope_section))
            ], axis=-1)
