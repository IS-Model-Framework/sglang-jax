...
def apply_rotary_pos_emb_vision(x: jax.Array, rotary_pos_emb: jax.Array) -> jax.Array:
    ...
    rotary_pos_emb = rotary_pos_emb.astype(jnp.float32)
...
class Qwen3_VisionRotaryEmbedding(nnx.Module):
    ...
    def __call__(self, seqlen: int) -> jax.Array:
        ...
        return freqs # NOTE (qihang)align with transformers
...
class Qwen3_VisionModel(nnx.Module):
    ...
    def fast_pos_embed_interpolate(self, grid_thw):
        # grid_thw: List of tuples or array [[t, h, w], ...]
        # 在 JAX 中为了性能，最好尽量向量化。但因为 h, w 是动态的，Python 循环是必须的。
        
        patch_pos_embeds_permute = []
        m_size = self.spatial_merge_size
        
        # 准备分片 spec，用于后续 reshape
        # 假设 dim 在 tensor 轴切分
        sharding_dim_sharded = NamedSharding(self.mesh, P(None, "tensor"))
        
        # [num_grid, dim]
        # 注意：这里不需要 transpose(1,0) 了，因为我们是用 index gather，直接取行
        # Transformers 源码: self.pos_embed(idx_tensor) -> [N, dim]
        embeds = self.pos_embed.embedding # [num_grid, dim]

        for t, h, w in grid_thw:
            h_idxs = jnp.linspace(0, self.num_grid_per_side - 1, h, dtype=jnp.float32)
            w_idxs = jnp.linspace(0, self.num_grid_per_side - 1, w, dtype=jnp.float32)
            ...
            def get_idx(r, c):
                return r * self.num_grid_per_side + c
            
            idx_tl = get_idx(h_idxs_floor, w_idxs_floor) # Top-Left
            idx_tr = get_idx(h_idxs_floor, w_idxs_ceil)  # Top-Right
            idx_bl = get_idx(h_idxs_ceil, w_idxs_floor)  # Bottom-Left
            idx_br = get_idx(h_idxs_ceil, w_idxs_ceil)   # Bottom-Right
            ...
            emb_tl = jnp.take(embeds, idx_tl, axis=0)
            emb_tr = jnp.take(embeds, idx_tr, axis=0)
            emb_bl = jnp.take(embeds, idx_bl, axis=0)
            emb_br = jnp.take(embeds, idx_br, axis=0)
            ...
            pos_embed = pos_embed.astype(self.dtype)

            target_sharding_5d = NamedSharding(self.mesh, P(None, None, None, None, "tensor"))
            
            pos_embed = jax.lax.reshape(
                pos_embed,
                (h // m_size, m_size, w // m_size, m_size, pos_embed.shape[-1]),
                out_sharding=target_sharding_5d
            )

            pos_embed = pos_embed.transpose(0, 2, 1, 3, 4)

            pos_embed = jax.lax.reshape(
                pos_embed,
                (pos_embed.shape[0] * pos_embed.shape[1] * pos_embed.shape[2] * pos_embed.shape[3], pos_embed.shape[-1]),
                out_sharding=sharding_dim_sharded
            )
    ...
    def __call__(self, x: jax.Array, grid_thw: tuple[tuple[int, int,
        ...
        rotary_pos_emb = self.rot_pos_emb(jnp.array(grid_thw).tolist())#几乎一致
...
class Qwen3VLForConditionalGeneration(nnx.Module):
    ...
    def __init__(
        ...
        self.dtype = dtype
        self.mesh = mesh
        self.model = Qwen3VLModel(self.config, mesh=mesh, dtype=dtype)
        ...
        self.is_mrope_enabled = "mrope_section" in config.text_config.rope_scaling if config.text_config.rope_scaling else False

        self.logits_processor = LogitsProcessor(config.text_config.vocab_size, mesh=mesh)
...
def load_model(device = "cpu", dtype=jnp.float32, model_path="/models/Qwen3-VL/Qwen3-VL-2B-Instruct"):
    import numpy as np
    import json
    file_path = f"{model_path}/config.json"
    with open(file_path, "r", encoding="utf-8") as f:
        data = json.load(f)  
    config = Qwen3VLConfig().from_dict(data)
    ...
    jax.set_mesh(mesh)
    model = Qwen3VLForConditionalGeneration(config, dtype, mesh=mesh)
    print("[SGLang-JAX] Model initialized successfully.")
    '''
    Model_path:
    ...
    '''
    model.load_weights(model_config=ModelConfig(model_path=model_path))
    print("[SGLang-JAX] Model Load Weight successfully.")
    return model


def compare_models(model_path, use_device, JAX_dtype, torch_dtype):
    # Create Input
    seq_len = 14080
    hidden_dim = 1536 
    # np_pixel_values = np.ones((seq_len, hidden_dim), dtype=np.float32)
    np_pixel_values = np.random.normal(0, 1, (seq_len, hidden_dim)).astype(np.float32)
    grid_thw = np.array([[4, 80, 44]], dtype=np.int32)

    # PyTorch Model 
    print("\n[PyTorch] Loading model...")
    pt_model = HF_Qwen3VLForConditionalGeneration.from_pretrained(
        model_path, dtype=torch_dtype, device_map=use_device
    )
    pt_model.eval()
    print("[PyTorch] Running forward pass...")
    pt_pixel_values = torch.tensor(np_pixel_values, dtype=torch_dtype).to(pt_model.device)
    pt_grid_thw = torch.tensor(grid_thw, dtype=torch.int64).to(pt_model.device)
    with torch.no_grad():
        pt_output = pt_model.get_image_features(pt_pixel_values, pt_grid_thw)
    # pt_output_np = pt_output[0].detach().float().cpu().numpy()
    pt_output_np = torch.cat([pt_output[0][0],pt_output[1][0],pt_output[1][1],pt_output[1][2]],dim=0).detach().float().cpu().numpy()

    # JAX Model
    jax_model = load_model(device=use_device, dtype=JAX_dtype, model_path=model_path)
    item = MultimodalDataItem(
        modality=Modality.IMAGE,
        feature=jnp.asarray(np_pixel_values, dtype=JAX_dtype),
        model_specific_data={
            "image_grid_thw": jnp.array(grid_thw, dtype=jnp.int32)
        }
    )
    item.set_pad_value() 
    jax_output = jax_model.get_image_feature([item])
    jax_output_np = np.array(jax_output, dtype=np.float32)

    # Check Difference
    print("\n[Comparison] Calculating differences...")
    if pt_output_np.shape != jax_output_np.shape:
        print(f"❌ Shape Mismatch! PyTorch: {pt_output_np.shape}, JAX: {jax_output_np.shape}")
        return

    diff = np.abs(pt_output_np - jax_output_np)
    ...
    if max_diff < tolerance:
        ...
        print("❌ Test Failed! Differences are too large.")
        print("\nFirst 5 values (PyTorch):", pt_output_np.flatten()[:5])
        print("First 5 values (JAX):    ", jax_output_np.flatten()[:5])

    q100, q75, q50 = np.percentile(diff, [100, 75, 50])
    print(f"Difference Quantiles:")
    print(f"  100% (Max) : {q100:.6f}")
    print(f"  75%        : {q75:.6f}")
...
if __name__ == "__main__":
    from transformers import Qwen3VLForConditionalGeneration as HF_Qwen3VLForConditionalGeneration
    import torch
    model_path = "/models/Qwen3-VL/Qwen3-VL-2B-Instruct"
    use_device = "cpu"
    JAX_dtype = jnp.float32
    torch_dtype = torch.float32
    compare_models(model_path, use_device, JAX_dtype, torch_dtype)