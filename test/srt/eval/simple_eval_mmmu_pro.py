"""
MMMU_Pro evaluation for VLMs using the run_eval simple-evals interface.
...
class MMMUPro(Eval):
    ...
    def __init__(
        self, num_examples: int | None = 100, num_threads: int = 32, seed: int = 42
    ):
        self.num_examples = num_examples
        self.num_threads = num_threads
        self.seed = seed
        # Prepare samples deterministically across all MMMU_Pro subjects (validation split)
    ...
    def _prepare_mmmu_samples(self, k: int) -> List[dict]:
        # Subjects and domains copied from MMMU_Pro data_utils to categorize results
        subjects: List[str] = []
        ...
        for subj in subjects:
            try:
                d = load_dataset("MMMU/MMMU_Pro", subj, split="test")
                ...
                continue
        if not datasets:
            raise RuntimeError("Failed to load MMMU_Pro datasets")
    ...
    def build_chat_messages_from_prompt(prompt: str, image_data) -> List:
        ...
        prefix, suffix = MMMUPro._split_prompt_for_image(prompt)
    ...
    def __call__(self, sampler: SamplerBase) -> EvalResult:
        def fn(sample: dict):
            ...
            prompt_messages = MMMUPro.build_chat_messages_from_prompt(
                prompt, image_data
            )
...
def _parse_multi_choice_response(
    ...
    # loosely adapted from benchmark mmmu_pro eval
    for char in [",", ".", "!", "?", ";", ":", "'"]:
